...
print("Load dataset")
f = gzip.open('./pkl/sem-relations-sdp-2018.pkl.gz', 'rb')
data = pkl.load(f)
f.close()
...
class CnnOneAttNet(nn.Module):
    ...
    def forward(self, words, pos1, pos2, sdp, labels, mask, idx, indices):
        ...
        sdp = F.relu((sdp.float()*0.6 + 0.8)/2)
...
optimizer = optim.SGD(model.parameters(), lr=3e-2, weight_decay=1e-4)
...
def test(epoch):
    ...
    #print(prec, f1)
    entity_pair = []
    with open('./answer/1.1.test.relations.txt', 'r', encoding='utf8') as f:
        lines = f.readlines()
        for line in lines:
...
if not os.path.isfile('./answer/keys.test.1.1.txt'):
...
for epoch in range(nb_epoch):
    ...
    os.system('./answer/eval.pl ./answer/propose_answer.txt ./answer/keys.test.1.1.txt > ./answer/result_2018.txt')
    best = 0
