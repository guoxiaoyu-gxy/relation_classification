...
class CnnOneAttNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv = nn.Conv1d(300+2*position_dims, nb_filter, kernel_size=filter_length, padding=1)
        self.drop = nn.Dropout(0.25)
        self.fc1 = nn.Linear(nb_filter, n_out)
        self.fc2 = nn.Linear(nb_filter+2*)
...
def get_lex(inputs, posIndex):
    ...
    noun1 = torch.mm(index_vec1, inputs)
    noun2 = torch.mm(index_vec2, inputs)
    print(noun1.shape)
    return torch.cat((noun1, noun2), 1)
