...
class CnnOneAttNet(nn.Module):
    def __init__(self):
        ...
        self.fc_f = nn.Linear(nb_filter, n_out)
    ...
    def forward(self, words, pos1, pos2):
        ...
        #output_1 = output[:,0:output.shape[1]-1,:]
        #output_2 = output[:,1:,:]
        
        lex = torch.max(F.relu(self.conv(output.permute(0, 2, 1))), 2)[0]
        #lex = self.fc_c(lex)

        #H = (o1_sen + o2_sen).permute(0, 2, 1)
