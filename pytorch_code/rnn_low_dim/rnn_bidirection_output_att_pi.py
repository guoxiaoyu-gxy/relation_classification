...
data = pkl.load(f1)
f1.close()
f2 = gzip.open('../pkl/sem-relations-low-dim-pi.pkl.gz', 'rb')
data_all = pkl.load(f2)

embeddings = data_all['wordEmbeddings']
yTrain, sentenceTrain, positionTrain1, positionTrain2, positionIndexTrain, sdpWeightTrain = data_all['train_set']
yTest, sentenceTest, positionTest1, positionTest2, positionIndexTest, sdpWeightTest = data_all['test_set']
...
print("sdpWeightTrain: ", sdpWeightTrain.shape)
...
print("sdpWeightTest: ", sdpWeightTest.shape)
...
class CnnOneAttNet(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(embedding_dims, hidden_dims, batch_first=True, dropout=0.3, bidirectional=True)
        #self.lstm = nn.LSTM(embedding_dims+2*position_dims, hidden_dims, batch_first=True, dropout=0.3, bidirectional=True)
        ...
        self.fc = nn.Linear(hidden_dims, n_out)
        self.softmax = nn.Softmax(1)
     
    def forward(self, words, pos1, pos2, sdp):
        embed1 = self.emb1(words)
        embed2 = self.emb2(pos1)
        embed3 = self.emb2(pos2)

        x = self.drop1(embed1)
        #x = self.drop1(torch.cat([embed1, embed2, embed3], 2))
        ...
        h1, h2 = output.chunk(2, dim=2)
        H = (h1 + h2).permute(0, 2, 1)
        alpha = self.softmax(torch.matmul(self.weight.unsqueeze(0), F.tanh(H)))
        sen = F.tanh(self.drop2(torch.bmm(H, alpha.permute(0, 2, 1)).squeeze(2)))
        #sen = torch.bmm(H, self.softmax(sdp).unsqueeze(2)).squeeze(2)
        x = self.fc(sen)
        return x
...
def train(epoch):
    ...
    sdp = generate(sdpWeightTrain, batch_size, indexes)
    for i in range(len(sentence)):
        sentence[i], position1[i], position2[i], labels[i], sdp[i] = Variable(torch.from_numpy(sentence[i])), \
            Variable(torch.from_numpy(position1[i])), Variable(torch.from_numpy(position2[i])), \
            Variable(torch.from_numpy(labels[i])), Variable(torch.from_numpy(sdp[i]))
        optimizer.zero_grad()
        output = model(sentence[i], position1[i], position2[i], sdp[i])
...
def test():
    ...
    sdp = generate(sdpWeightTest, test_batch_size, test_indexes)
