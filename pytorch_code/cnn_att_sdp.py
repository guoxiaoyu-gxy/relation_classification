...
class CnnOneAttNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv = nn.Conv1d(2*embedding_dims+2*position_dims, nb_filter, kernel_size=filter_length, padding=1)
        self.drop = nn.Dropout(0.25)
        self.fc = nn.Linear(nb_filter, n_out)
        self.softmax = nn.Softmax()

        self.U = nn.Parameter(torch.randn(embedding_dims, class_dims),requires_grad=True)
        self.class_matrix = nn.Parameter(torch.randn(class_dims, n_out), requires_grad=True)
        self.M = nn.Parameter(torch.randn(max_sentence_len, n_out), requires_grad=True)
        
    def forward(self, words, pos1, pos2, posIndex, sdpMatrix):
        emb1 = self.emb1(words)
        emb2 = self.emb2(pos1)
        emb3 = self.emb2(pos2)
        #rela = add_att(emb1, posIndex)

        G = torch.matmul(emb1, self.U)
        G = torch.matmul(G, self.class_matrix)
        #print(G.shape)
        h = torch.matmul(sdpMatrix.view(-1, 1, max_sentence_len), self.M)
        #print(h.shape)
        h = torch.bmm(h, G.transpose(1,2))
        #print(h.shape)
        u = torch.add(sdpMatrix, multiple*torch.squeeze(h,1)).view(-1, max_sentence_len, 1)
        #print(u.shape)
        R = u*emb1
        #print(R.shape)
        x = torch.cat([emb1, emb2, emb3, R], 2).permute(0, 2, 1)
...
def train(epoch):
    ...
    sdpWeight = generate(sdpWeightTrain, batch_size, indexes)
    for i in range(len(sentence)):
        sentence[i], position1[i], position2[i], labels[i], positionIndex[i], sdpWeight[i] = \
            ...
            Variable(torch.from_numpy(positionIndex[i])), Variable(torch.from_numpy(sdpWeight[i]))
        optimizer.zero_grad()
        output = model(sentence[i], position1[i], position2[i], positionIndex[i], sdpWeight[i])
...
def test():
    ...
    sdpWeight = generate(sdpWeightTest, test_batch_size, test_indexes)
    for i in range(len(sentence)):
        sentence[i], position1[i], position2[i], labels[i], positionIndex[i], sdpWeight[i] = \
            Variable(torch.from_numpy(sentence[i])), Variable(torch.from_numpy(position1[i])), \
            Variable(torch.from_numpy(position2[i])), Variable(torch.from_numpy(labels[i])), \
